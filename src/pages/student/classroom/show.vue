  ...
  methods: {
    ...
    //开始回答
      answerQuestions(id, content, pic) {
          if(this.ready){
              ...
              navigator.camera.getPicture(
                  function(imageData){
                      let i = _this.objectiveAnswer.findIndex(e=>{
                          return e.id === _this.subjectiveId;
                      });
                      console.log(i);
                      if(i != -1){
                          let answer = _this.objectiveAnswer[i];
                          answer.answer = imageData;
                          _this.$set(_this.objectiveAnswer, i, answer);
      ...
      async sumbitSubjectPic(data){
          ...
          //主观题答案存根
          await sendSubjectPicByString(data)
              .then(res => {
                  Toast({
                      message: "图片存储成功！",
                      position: "middle",
      ...
      .subjective_warp {
        ...
        .button_warp {
          ...
          .Answerstart {
            ...
            height: 46px;
            background: url('../../../assets/按钮.png') no-repeat;//
            background-position: -135px -3121px;
          }
          .Answerstart:active{
            background: url('../../../assets/按钮.png') no-repeat;
